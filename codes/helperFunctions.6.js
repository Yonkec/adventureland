function loadCharacters(){
	start_character("Chunior", "Main");
	start_character("Blumps", "Main");
	start_character("Nuggs", "Main");
	log("Loading Characters...");
	setTimeout(initParty, 8000);
}

function initParty(){
	send_party_invite("Chunior");
	send_party_invite("Blumps");
	send_party_invite("Nuggs");
	log("Party Invites sent!");
}

function stopCharacters(){
	stop_character("Chunior");
	stop_character("Blumps");
	stop_character("Nuggs");
...
function on_party_invite(name) {
	log(character.name + ': on_party_invite start');
	if (get_active_characters()[name]) {
		log(character.name + ': on_party_invite inside!');
		accept_party_invite(name);
	}
	log(character.name + ': on_party_invite end');
}
